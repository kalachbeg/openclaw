...
health.start();
// IMPORTANT: Always call stop() when shutting down to avoid timer leaks
health.stop();
...
**Note:**  
Always call `health.stop()` when the bot is shutting down to avoid resource leaks. The health check implementation now ensures timeouts are cleared on success.
